


var local_database = [
  {
    post_date: 'Sep 25 2016',
    post_title: '西伯利亚-67℃',
    post_content: '俄罗斯西伯利亚的奥伊米亚康村（Oymyakon）是地球上最寒冷的人类居住地，当地近日又测得极寒冷的零下67摄氏度，几乎追平1933年测到的零下67.7度纪录，冷到连温度计都被冻坏。',
    post_like: '92',
    post_read: '68',
    img_condition: true,
    post_img: '/images/ice.jpg',
    post_author_img: '/images/user-icon.png',
    postid: 0,

    headImgSrc: '/images/ice.jpg',
    author: '林白衣',
    date_time: '24小时前',
    detail: '近日，位于俄罗斯西伯利亚东北部的奥伊米亚康村测得摄氏零下67度的极寒天气，几乎追平1933年测到的零下67.7度记录，居民外出时连睫毛也结了一层厚厚的霜。 据英国《每日邮报》报道，俄罗斯西伯利亚的奥伊米亚康村（Oymyakon）是地球上最寒冷的人类居住地，当地近日又测得极寒冷的摄氏零下67度，几乎追平1933年测到的零下67.7度纪录，冷到连温度计都被冻坏。当地冰天雪地一片，村民走出屋外时，冷到连眼睫毛都会结霜。\n    当地居民说，村子里先前装了一具新的电子式温度计，不过因为天气实在太冷，电子温度计已经宣告失灵，萤幕上显示零下62度，但实际气温还要更低。奥伊米亚康位于俄罗斯东北部的萨哈共和国（Sasha Republic），当地曾在1933年时测得零下67.7度的极端低温，同时也是北半球出现过的最低温纪录。  \n  　　极端低温为当地居民带来诸多不便，例如他们用的笔可能会结冰，眼镜也会直接在脸上结成冰，车子熄火后就很难再发动，因此车主干脆一整天都不熄火。（海外网 张振）',

    music: {
      src: 'http://ws.stream.qqmusic.qq.com/M500001VfvsJ21xFqb.mp3?guid=ffffffff82def4af4b12b3cd9337d5e7&uin=346897220&vkey=6292F51E1E384E061FF02C31F716658E5C81F5594D561F2E88B854E81CAAB7806D5E4F103E55D33C16F3FAC506D1AB172DE8600B37E43FAD&fromtag=46',
      coverImgUrl: 'http://y.gtimg.cn/music/photo_new/T002R300x300M000003rsKF44GyaSk.jpg?max_age=2592000',
      title: '此时此刻'
    }

  },
  {
    post_date: 'Sep 26 2016',
    post_title: '日本海岸再现“幽灵船”',
    post_content: '据海外网报道，日本石川县金泽西警署16日透露，在漂流至金泽市下安原町海岸的木船中发现7具不明身份的男尸，警方将展开详细调查。',
    post_like: '92',
    post_read: '68',
    img_condition: true,
    post_img: '/images/ghost.jpg',
    post_author_img: '/images/user-icon.png',
    postid: 1,
    headImgSrc: '/images/ghost.jpg',
    author: '海外网',
    date_time: '2小时前',
    detail: '据海外网报道，日本石川县金泽西警署16日透露，在漂流至金泽市下安原町海岸的木船中发现7具不明身份的男尸，警方将展开详细调查。\n据日本共同社报道，金泽西警署透露，7具尸体是15日在船内发现的，都穿着衣服，未发现明显外伤等。尸体已开始腐败，可能已死亡一段时间，警方将通过司法解剖查明死因。10日上午，木船被发现时处于倾覆状态，由于天气恶劣没有立即对船内进行搜查。    \n据悉，木船全长约16米，内部有多个小房间。前部的房间和中央的房间分别发现4具和3具尸体，各自重叠在一起。船内还发现了1枚徽章和印有谚文的香烟盒及绳网。    \n10日上午，在距船约15米的沙滩上也发现了男尸，该警署和金泽海上保安部正在调查是否与木船有关。该男性的司法解剖结果是年龄约在30到60岁之间，在去年9月前后死亡。    \n海外网早前报道，日本多地去年至今已经接连出现了多起“幽灵船”抵岸的情况。据日本海上保安厅称，2017年“幽灵船”事件达到104起，是2013年开始统计以来最多的一年。今年截至1月15日已确认了8起。',

    music: {
      src: 'http://ws.stream.qqmusic.qq.com/C100003507bR0gDKBm.m4a?fromtag=38',
      coverImgUrl: 'http://y.gtimg.cn/music/photo_new/T002R300x300M000003rsKF44GyaSk.jpg?max_age=2592000',
      title: '齐秦-夜夜夜夜'
    }
  },
];

var local_movies = [
  {
    "title": "正在热映",
    "more": "更多",
    "image_go": "/images/箭头.png",
    "movies": [
      {
        "movie_name": "机器人9号",
        "movie_image": "http://img3.doubanio.com\/view\/photo\/s_ratio_poster\/public\/p494268647.webp",
        "movie_score": "7.4"
      },
      {
        "movie_name": "三傻大闹宝莱坞",
        "movie_image": "http://img3.doubanio.com\/view\/celebrity\/s_ratio_celebrity\/public\/p13628.webp",
        "movie_score": "9.1"
      },
      {
        "movie_name": "人再囧途之泰囧",
        "movie_image": "http://img3.doubanio.com\/view\/celebrity\/s_ratio_celebrity\/public\/p43738.webp",
        "movie_score": "9.2"
      }
    ]

  },
  {
    "title": "即将上映",
    "more": "更多",
    "image_go": "/images/箭头.png",
    "movies": [
      {
        "movie_name": "大话西游之大圣娶亲",
        "movie_image": "http://img3.doubanio.com\/view\/photo\/s_ratio_poster\/public\/p2455050536.webp",
        "movie_score": "7.4"
      },
      {
        "movie_name": "人在囧途",
        "movie_image": "http://img3.doubanio.com\/view\/photo\/s_ratio_poster\/public\/p500548437.webp",
        "movie_score": "8.9"
      },
      {
        "movie_name": "大话西游之月光宝盒",
        "movie_image": "http://img3.doubanio.com\/view\/photo\/s_ratio_poster\/public\/p1280323646.webp",
        "movie_score": "8.7"
      }
    ]
  },
  {
    "title": "全球榜",
    "more": "更多",
    "image_go": "/images/箭头.png",
    "movies": [
      {
        "movie_name": "让子弹飞",
        "movie_image": "http://img3.doubanio.com\/view\/photo\/s_ratio_poster\/public\/p1512562287.webp",
        "movie_score": "7.0"
      },
      {
        "movie_name": "羞羞的铁拳",
        "movie_image": "http://img3.doubanio.com\/view\/photo\/s_ratio_poster\/public\/p2499793218.webp",
        "movie_score": "8.4"
      },
      {
        "movie_name": "唐伯虎点秋香",
        "movie_image": "http://img7.doubanio.com\/view\/photo\/s_ratio_poster\/public\/p1946455272.webp",
        "movie_score": "7.4"
      }
    ]
  },
  {
    "title": "内地榜",
    "more": "更多",
    "image_go": "/images/箭头.png",
    "movies": [
      {
        "movie_name": "夏洛特烦恼",
        "movie_image": "http://img3.doubanio.com\/view\/celebrity\/s_ratio_celebrity\/public\/p1356510694.28.webp",
        "movie_score": "7.1"
      },
      {
        "movie_name": "西游降魔篇",
        "movie_image": "http://img7.doubanio.com\/view\/celebrity\/s_ratio_celebrity\/public\/p28063.webp",
        "movie_score": "6.8"
      },
      {
        "movie_name": "美人鱼",
        "movie_image": "http://img3.doubanio.com\/view\/photo\/s_ratio_poster\/public\/p2316177058.webp",
        "movie_score": "8.9"
      }
    ]
  }
];



module.exports = {
  postList: local_database,
  local_movies: local_movies
}